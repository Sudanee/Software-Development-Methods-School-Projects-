...
void print_formatted_event(char *, char *, char *, char *, int);
...
void print_formatted_date(char *date){
    ...
    int i = 0;
    while (date[i] != 0){ // for every char in given date prints "-"
        printf("-");
        i++;
    }
    printf("\n");
 ...
 *
 */
void print_formatted_event(char *start_time, char *end_time, char *description, char *location, int first_event){
    // TODO: Complete the function.
    printf("%s to %s: %s {{%s}}", start_time, end_time, description, location);
    if (first_event == 0){ // prints for last entry
        printf("\n");
    }
}
    ...
    while (curr != NULL)
    {
        ...
        if (is_date_in_range(curr->val->start_date, from_date, to_date)){
            ...
            if (compare_time(current_date, curr->val->start_date, 0) != 0)
            {
                ...
                print_formatted_event(start_time, end_time, curr->val->description, curr->val->location, first_event);
            }
